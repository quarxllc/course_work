...
float biker_with_airdrag(int hill, float P, float mass, float velocity, float dt, float area, float rho, float theta ){

	if (hill == 1){
		return  velocity + (P/(mass * velocity) + sin(theta) * gravity - (0.5) * (rho * area)/ mass * pow(velocity,2)) * dt;
}
	else if (hill == 2){
		return  velocity + (P/(mass * velocity) - sin(theta) * gravity - (0.5) * (rho * area)/ mass * pow(velocity,2)) * dt;
	}
...
int main(int argc, char **argv){
	...
	float  mass0 = 70, area0 = 0.33, power0 = 400, power, area, mass, dp, angle; 
	float *terminal, rho = 1.21, percent, percent1, percent2;
	int t,i,j, seconds = 100000, hill;
	float *time = NULL;
	float *velocity = NULL;
	...
	cout << "Enter 1 for downhill and 2 for downhill: " << endl;
	cin >> hill;

	while(cin.fail() || hill == 0 || hill > 2){ 
	...
	cout << "Enter your angle of the hill in degrees: " << endl;
	cin >> angle;
	...
	for (size_t output_file_number = 0; output_file_number < (size_t)i; output_file_number++){
		...
		for ( t = 1; t < seconds; t++){
			...
			velocity_with_airdrag[t] = biker_with_airdrag(hill, power, mass,\
					velocity_with_airdrag[t-1], dt[output_file_number], area, rho, angle);
